...
def main(config):
    ...
    if ckpt_name is None:
        ...
        ckpt_name += '_{}_way_{}_shot'.format(
            config['train']['n_way'], config['train']['k_shot'])
    ...
    utils.log('meta-train set: {} (x{}), {}'.format(
        train_set[0][0].shape, len(train_set), train_set.n_cls))
    ...
    if config.get('val'):
        ...
        utils.log('meta-val set: {} (x{}), {}'.format(
            val_set[0][0].shape, len(val_set), val_set.n_cls))
    ...
    for epoch in range(start_epoch, config['epoch'] + 1):
        ...
        for data in tqdm(train_loader, desc='meta-train', leave=False):
            ...
            x_shot, y_shot = x_shot.float().cuda(), y_shot.cuda()
            x_query, y_query = x_query.float().cuda(), y_query.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',
                        ...
                        default='./configs/cwru_wdcnn_train.yaml',
                        )
