...
from .modules import get_child_dict, Module, BatchNorm1d
...
class MAML(Module):
    ...
    def _inner_iter(self, x, y, params, mom_buffer, episode, inner_args, detach):
        ...
        Args:
          x (float tensor, [n_way * n_shot, C, L]): per-episode support set.
    ...
    def _adapt(self, x, y, params, episode, inner_args, meta_train):
        ...
        for m in self.modules():
            if isinstance(m, BatchNorm1d) and m.is_episodic():
    ...
    def forward(self, x_shot, x_query, y_shot, inner_args, meta_train):
        ...
        for ep in range(x_shot.size(0)):
            ...
            if not meta_train:
                for m in self.modules():
                    if isinstance(m, BatchNorm1d) and not m.is_episodic():
                        m.eval()
